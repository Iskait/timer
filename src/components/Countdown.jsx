...
import { togglecountdownOn, timerRestart, timerTickBack, timerCancel, setPercentage } from '../redux/slices/countdownSlice';
...
function Countdown() {  
    ...
    useEffect(() => {
        ...
        if (countdownOn) {
            ...
            dispatch(setPercentage({percentage:((timerSeconds-1) / allSeconds) * 100}));
            }, 1000)
        } else {
            clearInterval(timer);
        }
    ...
    return (
        <div className='timer__body'>
            <div className="timer__countdown">
                ...
                <button className="timer__cancel" onClick={handleCancel}>Cancel</button>
                </>
                }
                </div>
                <CircularProgressbar 
            ...
            <>
            <button className="timer__cancel" onClick={handleCancel}>Cancel</button>
            <button className="timer__toggler" onClick={() => dispatch(togglecountdownOn())}>{countdownOn ? 'Stop' : 'Resume'}</button>
            </> : 
